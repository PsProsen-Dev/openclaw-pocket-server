...
if [ -f "$PROJECT_DIR/.glibc-arch" ]; then
    echo -e "  ${GREEN}[OK]${NC}   glibc"
else
    echo -e "  ${YELLOW}[OLD]${NC} Bionic (pre-2.0.0) - run 'oca --update' to migrate"
fi

if [ "${OCA_GLIBC:-}" = "1" ]; then
    echo -e "  ${GREEN}[OK]${NC}   OCA_GLIBC=1 (environment)"
else
    echo -e "  ${YELLOW}[MISS]${NC} OCA_GLIBC not set - run 'source ~/.bashrc'"
fi
...
if [ -d "$PROJECT_DIR" ]; then
    echo "  ~/.oca:               $(du -sh "$PROJECT_DIR" 2>/dev/null | cut -f1)"
fi
