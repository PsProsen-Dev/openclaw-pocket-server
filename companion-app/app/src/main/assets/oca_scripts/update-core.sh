...
echo ""
echo -e "${BOLD}========================================${NC}"
echo -e "${BOLD}  OpenClaw on Android - Updater v${OCA_VERSION}${NC}"
echo -e "${BOLD}========================================${NC}"
echo ""
...
step 1 "Analyzing RTX⚡ Protocol Readiness"
...
# Handle legacy migration if a user somehow had .openclaw-lite or .openclaw-android
OLD_DIR="$HOME/.openclaw-android"
...
# Define REPO_TARBALL after sourcing lib.sh
...
if [ "$SDK_INT" -ge 31 ] 2>/dev/null; then
    ...
    echo "       see: https://github.com/PsProsen-Dev/OpenClaw-On-Android/blob/main/docs/disable-phantom-process-killer.md"
fi

step 2 "Download Latest Release (tarball)"

mkdir -p "$PREFIX/tmp"
RELEASE_TMP=$(mktemp -d "$PREFIX/tmp/oca-update.XXXXXX") || {
...
step 3 "Syncing RTX⚡1 Infrastructure"
...
cp "$RELEASE_TMP/oca.sh" "$PREFIX/bin/oca"
chmod +x "$PREFIX/bin/oca"

cp "$RELEASE_TMP/update.sh" "$PREFIX/bin/ocaupdate"
...
step 4 "Synchronizing RTX⚡2 Platform Engine"
...
step 5 "Upgrading RTX⚡3 Master Arsenal"
