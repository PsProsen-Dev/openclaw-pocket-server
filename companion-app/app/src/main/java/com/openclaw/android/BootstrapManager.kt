...
"""
    ...
    private fun copyAssetScripts() {
        ...
        // Copy OCA scripts to the environment
        val ocaScriptsDir = File(ocaDir, "oca_scripts")
        copyAssetDirectory("oca_scripts", ocaScriptsDir)
    }
        ...
        try {
            ...
            for (asset in assets) {
                ...
                if (subAssets != null && subAssets.isNotEmpty()) {
                    copyAssetDirectory(fullAssetPath, targetFile)
                } else {
                    copyBundledAsset(fullAssetPath, targetFile)
                    if (asset.endsWith(".sh") || asset.startsWith("oca") || asset.startsWith("install")) {
                        targetFile.setExecutable(true)
                    }
                }
            }
        } catch (e: Exception) {
            AppLogger.w(TAG, "Failed to copy asset directory $assetPath", e)
        }
    }
