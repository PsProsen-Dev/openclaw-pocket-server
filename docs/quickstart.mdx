...
## 🆓 How to Get Free AI Models Fast

Want GPT-4.1 or GPT-5 mini at ₹0 cost? Go to your CLI/app’s `/models` menu, pick any model with a green tick (usually GPT-4.1, GPT-5 mini, MiniMax, Kimi, Big Pickle, etc), then provide your free [Zen API key](https://zen.opencode.ai/).

- 
- Each provider has quotas (e.g., 200 requests per 5 hours)
- Only for dev/hobby/test—free plans allow data for model improvement (avoid secrets/IP)
- Model availability changes—always check model picker or Zen dashboard!

**(See README.md for advanced pro-tips, full table, and upgrade info)**
