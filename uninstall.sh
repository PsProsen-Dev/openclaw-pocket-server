...
echo ""
echo -e "${BOLD}========================================${NC}"
echo -e "${BOLD}  OpenClaw on Android - RTX⚡ Purge System${NC}"
echo -e "${BOLD}========================================${NC}"
echo ""
...
step 1 "Decomposing RTX⚡2 Platform Engine"
...
step 2 "Purging RTX⚡3: code-server"
...
step 4 "oca and ocaupdate commands"
if [ -f "${PREFIX:-}/bin/oca" ]; then
    rm -f "${PREFIX:-}/bin/oca"
    echo -e "${GREEN}[OK]${NC}   Removed ${PREFIX:-}/bin/oca"
else
    echo -e "${YELLOW}[SKIP]${NC} ${PREFIX:-}/bin/oca not found"
fi

if [ -f "${PREFIX:-}/bin/ocaupdate" ]; then
    rm -f "${PREFIX:-}/bin/ocaupdate"
    echo -e "${GREEN}[OK]${NC}   Removed ${PREFIX:-}/bin/ocaupdate"
else
    echo -e "${YELLOW}[SKIP]${NC} ${PREFIX:-}/bin/ocaupdate not found"
fi

step 5 "Removing RTX⚡2 Runtime (Glibc)"
...
step 7 "Final RTX⚡ Protocol Clearance"

if [ -d "$PROJECT_DIR" ]; then
    if ask_yn "Remove installation directory (~/.oca)? Includes Node.js, patches, configs."; then
        rm -rf "$PROJECT_DIR"
