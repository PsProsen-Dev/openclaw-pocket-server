#!/usr/bin/env bash
# backup.sh — oca --backup / oca --restore implementation
# Sourced by oca.sh after lib.sh is loaded.
...
cmd_backup() {
    ...
    local tmpdir
    tmpdir=$(mktemp -d "${TMPDIR:-/tmp}/oca-backup.XXXXXX")
...
cmd_restore() {
    ...
    if [ ! -d "$BACKUP_DIR" ]; then
        ...
        echo -e "       Run ${BOLD}oca --backup${NC} first."
        exit 1
    fi
