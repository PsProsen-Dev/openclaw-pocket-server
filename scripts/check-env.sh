...
if command -v node &>/dev/null; then
    ...
    if [ "$NODE_MAJOR" -lt 24 ] 2>/dev/null; then
        echo -e "${YELLOW}[WARN]${NC} Node.js >= 24 required. Will be upgraded during install."
    fi
else
...
if [ "$SDK_INT" -ge 31 ] 2>/dev/null; then
    ...
    echo "       see: https://github.com/PsProsen-Dev/OpenClaw-On-Android/blob/master/docs/disable-phantom-process-killer.md"
fi

# Root detection
if su -c "id" &>/dev/null 2>&1; then
    echo -e "${GREEN}[OK]${NC}   Root access: detected (su available)"
else
    echo -e "${YELLOW}[INFO]${NC} Root access: not available (non-rooted device)"
fi

# Termux:API check
if command -v termux-battery-status &>/dev/null; then
    echo -e "${GREEN}[OK]${NC}   Termux:API: installed"
else
    echo -e "${YELLOW}[INFO]${NC} Termux:API: not found (install from F-Droid for extra features)"
fi
