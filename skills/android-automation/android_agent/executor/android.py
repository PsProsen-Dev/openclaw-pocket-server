...
class AndroidExecutor(Executor):
    ...
    def move_mouse(self, x: int, y: int, observation: str) -> bool:
        try:
            ...
            logger.debug(
                f"move mouse x y {x} {y} -> scaled {scaled_x} {scaled_y} (scale_factor: {self.image_scale_factor})"
            )
    ...
    def click_at_a_point(self, x: int, y: int, observation: str) -> bool:
        try:
            ...
            scaled_y = int(y * self.image_scale_factor)
            logger.debug(
                f"click at a point x y {x} {y} -> scaled {scaled_x} {scaled_y} (scale_factor: {self.image_scale_factor})"
            )
            run_adb_command(["shell", "input", "tap", str(scaled_x), str(scaled_y)])
            return True
        except Exception as e:
    ...
    def screenshot(
        ...
        try:
            ...
            if hasattr(self, "image_quality") and self.image_quality < 100:
                ...
                logger.debug(
                    f"Resized screenshot from original to {new_width}x{new_height} (quality: {self.image_quality}%, scale_factor: {self.image_scale_factor})"
                )
            else:
    ...
    def gesture(
        ...
        """
        Execute a swipe/drag gesture via ADB input swipe.

        Args:
            x1: Start x coordinate (real screen pixels).
            y1: Start y coordinate (real screen pixels).
            x2: End x coordinate (real screen pixels).
            y2: End y coordinate (real screen pixels).
            duration_ms: Gesture duration in milliseconds.
                         100–200 for scroll/fling, 700–900 for slider drag.

        Returns:
            True on success, False on exception.
        """
        try:
            logger.debug(f"gesture ({x1},{y1}) -> ({x2},{y2}) {duration_ms}ms")
            run_adb_command(
                [
                    "shell",
                    "input",
            ...
            logger.exception("Error in gesture")
            return False
